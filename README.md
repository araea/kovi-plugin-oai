...
- 🤖 **多智能体管理** - 创建、复制、删除、配置多个智能体
- 💬 **对话模式** - 支持公有/私有对话，独立历史记录
- 🖼️ **多模态支持** - 图片识别、图片生成、自动预览
- 📝 **文本/图片输出** - 自由切换 Markdown 渲染或纯文本
- 📚 **历史管理** - 查看、编辑、删除、导出对话记录
- ⚡ **符号指令** - 简洁高效的指令语法
...
3. 在 `src/main.rs` 中添加 `kovi_plugin_oai`

## 快速开始

1. 发送 `API地址 API密钥` 配置 API（如 `https://api.openai.com/v1 sk-xxx`）
2. 发送 `#助手 gpt-4o 你是一个有帮助的助手` 创建智能体
3. 发送 `助手 你好` 开始对话
4. 发送 `oai` 查看完整帮助
